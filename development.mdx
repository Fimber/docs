...
If the **live site** shows a blockquote such as “Documentation Index” or “Fetch the complete documentation index at … `/llms.txt`” on every page, that UI is produced by **Mintlify’s hosted shell**, not by MDX in this repository. The crawl hint file can remain at `/llms.txt` (and `/.well-known/llms.txt`) for tools without advertising it as a human-facing banner—toggle or report that behavior in the [Mintlify dashboard](https://dashboard.mintlify.com) for your deployment (AI / ingestion or support), since there is no supported `docs.json` switch in our project for that banner at the time of writing.
