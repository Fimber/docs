...
This project still exposes the machine-readable index at **`/llms.txt`** (and `/.well-known/llms.txt`) and links it from the **site footer** under Tools. `docs.json` also sets **`contextual.options` to `[]`** so the header contextual menu stays minimal when the CLI supports that field.
