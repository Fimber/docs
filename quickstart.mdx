...
<Tabs>
  ...
  <Tab title="Linux">
    For a **known ImageMagick 7** CLI on Linux, use the **official AppImage** from [ImageMagick on GitHub Releases](https://github.com/ImageMagick/ImageMagick/releases). Open the release you want, expand **Assets**, and pick the file for your CPU (for example `ImageMagick-*-gcc-x86_64.AppImage` on Intel/AMD, or the arm64 AppImage on ARM).

    **Working example (replace when this version ages out):** the URL below points at a real asset; when you upgrade, copy the new **Download** URL from GitHub for that asset—filenames change every release.
    ...
    Always confirm the asset still exists on the [releases page](https://github.com/ImageMagick/ImageMagick/releases); if `wget` returns 404, pick the current `ImageMagick-*-gcc-x86_64.AppImage` link from the latest **7.x** release.
