...
<Tabs>
  ...
  <Tab title="Linux">
    Download an **ImageMagick 7** AppImage from the [GitHub releases](https://github.com/ImageMagick/ImageMagick/releases) page. Pick the asset that matches your CPU (for example `ImageMagick-*-gcc-x86_64.AppImage` on Intel/AMD, or the arm64 build on Apple Silicon Linux VMs). Copy the **exact download URL** from the release you want—do not reuse version numbers from older blog posts or docs, because filenames change every release.

    ```bash
    wget -O magick-appimage "PASTE_RELEASE_ASSET_URL_HERE"
    chmod +x magick-appimage
    sudo mv magick-appimage /usr/local/bin/magick
    magick --version
    ```

    The AppImage is self-contained (no `apt install` of ImageMagick required for that binary).

    On Ubuntu 26.04 and later, the `imagemagick` package from `apt` may include version 7. Check with `magick --version` after installing. On Ubuntu 24.04 and earlier, `apt install imagemagick` often installs version 6 only; use a current AppImage or another [official install path](https://imagemagick.org/script/download.php#linux) if you need the `magick` CLI from version 7.
  </Tab>
  <Tab title="Windows">
...
You can now install, convert, resize, inspect, and batch-process images. Three paths forward depending on what you need:
...
- **[Geometry syntax reference](/reference/geometry)** explains every modifier (`>`, `<`, `!`, `^`, `#`) you can append to dimension strings.